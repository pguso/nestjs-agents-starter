...
```
src/
  agents/     what the agent is for (instructions, tools, step limits)
...
|-------|------|----------------|
| `agents/` | Instructions, which tools an agent gets, step limits | HTTP routes, persistence, provider API keys |
...
sequenceDiagram
  ...
  ChatController->>ChatService: streamChat(ctx, messages, ...)
  ChatService->>AssistantAgent: create(ctx)
  AssistantAgent->>Tool: build(ctx) tools
  ChatService->>Client: AI SDK UI message stream
  ChatService->>ConversationStore: save if conversationId set
```

1. [`AuthGuard`](../../src/common/auth.guard.ts) reads `x-user-id` (default `demo-user`) and attaches `RequestContext`.
...
3. [`ChatService`](../../src/chat/chat.service.ts) builds the agent and pipes the UI stream to the response.
...
5. If `conversationId` was sent, finished messages are saved via [`CONVERSATION_STORE`](../../src/chat/conversation-store.ts).

`GET /conversations/:id` only hits the store and returns JSON — no agent run.

## Module wiring

[`AppModule`](../../src/app.module.ts) imports, in order of dependency:

`ConfigModule` → `CommonModule` → `ModelModule` → `ToolsModule` → `AgentsModule` → `ChatModule`
